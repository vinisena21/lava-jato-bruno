...
export function RelatorioFechamento({ veiculos }: RelatorioProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{
              backgroundColor: '#eff6ff', width: '36px', height: '36px', borderRadius: '10px',
          ...
          <span style={{
            ...
            padding: '6px 14px', borderRadius: '30px', border: '1px solid #bfdbfe',
      ...
      {/* NOVA TABELA DE VEÍCULOS REGISTRADOS */}
      <div style={{
        backgroundColor: '#ffffff',
        padding: '24px',
        borderRadius: '24px',
        ...
        {veiculos.length === 0 ? (
          <p style={{ color: '#64748b', fontSize: '14px', textAlign: 'center', padding: '20px 0' }}>Nenhum veículo registrado no período selecionado.</p>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px', textAlign: 'left' }}>
              <thead>
                <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                  <th style={{ padding: '12px', fontWeight: '700', color: '#475569' }}>Data</th>
                  <th style={{ padding: '12px', fontWeight: '700', color: '#475569' }}>Placa</th>
                  <th style={{ padding: '12px', fontWeight: '700', color: '#475569' }}>Modelo</th>
                  <th style={{ padding: '12px', fontWeight: '700', color: '#475569' }}>Valor</th>
                  <th style={{ padding: '12px', fontWeight: '700', color: '#475569' }}>Status</th>
                </tr>
              </thead>
              <tbody>
                {veiculos.map((v, index) => (
                  <tr key={v.id || index} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    <td style={{ padding: '12px', color: '#0f172a' }}>
                      {new Date(v.created_at || new Date()).toLocaleDateString('pt-BR')}
                    </td>
                    <td style={{ padding: '12px', fontWeight: 'bold', color: '#1e293b', textTransform: 'uppercase' }}>
                      {v.placa}
                    </td>
                    <td style={{ padding: '12px', color: '#475569', textTransform: 'capitalize' }}>
                      {v.modelo}
                    </td>
                    ...
                    <td style={{ padding: '12px' }}>
                      {v.pago ? (
                        <span style={{ backgroundColor: '#dcfce7', color: '#166534', padding: '4px 8px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold' }}>PAGO</span>
                      ) : (
                        <span style={{ backgroundColor: '#fee2e2', color: '#991b1b', padding: '4px 8px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold' }}>PENDENTE</span>
                      )}
                    </td>
                  </tr>
                ))}
